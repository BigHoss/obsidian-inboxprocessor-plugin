...
`).map(w=>w.trim()).filter(w=>w.length>0);if(p.length===0){new a.Notice("Inbox is clean \u2014 no links to process"),this.refreshStatusBar();return}let u=new Map;for(let w of this.settings.templates){let v=this.resolveFile(w.templatePath);v&&u.set(w.linkType,await this.app.vault.read(v))}let b=this.resolveFile(this.settings.defaultTemplatePath),y=b?await this.app.vault.read(b):W,m=[],h=[],f=0,d=0,g=0,x=!1,T=Math.min(p.length,this.settings.maxLinksPerRun);for(let w=0;w<T;w++){let v=p[w],A=M(v);if(!A){h.push(v);continue}try{let S=this.settings.showFetchNotices&&!n?k=>new a.Notice(`Inbox: ${w+1}/${T} \u2014 ${k}`,3e3):void 0,P=await this.processOne(A,u,y,S);if(P===null)h.push(v),d++;else if(typeof P=="object"&&"abort"in P){for(let k=w;k<T;k++)h.push(p[k]);for(let k=T;k<p.length;k++)h.push(p[k]);x=!0;break}else m.push(v),f++}catch(S){let P=S instanceof Error?S.message:String(S);n||new a.Notice(`\u2717 ${A.url} \u2014 ${P}`,8e3),h.push(v),g++,await $(this.app,this.manifest,A.url,P),await ct(this.settings,`Failed: ${A.url}
${P}`)}}if(!x)for(let w=T;w<p.length;w++)h.push(p[w]);let D=h.length>0?`
`+h.join(`
`)+`
`:`
`,I=r+D;await this.app.vault.modify(s,I),this.lastRunAt=Date.now(),n||new a.Notice(`Inbox: ${f} processed, ${d} skipped, ${g} kept for retry${T<p.length&&!x?`, ${p.length-T} deferred`:""}${x?" (aborted)":""}`),this.refreshStatusBar()}async processSingleLine(e){let n=M(e.trim());if(!n){new a.Notice("Current line is not a recognized link");return}let s=new Map;for(let r of this.settings.templates){let c=this.resolveFile(r.templatePath);c&&s.set(r.linkType,await this.app.vault.read(c))}let i=this.resolveFile(this.settings.defaultTemplatePath),l=i?await this.app.vault.read(i):W;try{let r=this.settings.showFetchNotices?p=>new a.Notice(p,3e3):void 0,c=await this.processOne(n,s,l,r);c===null?new a.Notice("Skipped duplicate"):typeof c=="object"&&"abort"in c?new a.Notice("Aborted"):new a.Notice(`\u2713 ${c}`),this.refreshStatusBar()}catch(r){let c=r instanceof Error?r.message:String(r);new a.Notice(`\u2717 ${n.url} \u2014 ${c}`,8e3),await $(this.app,this.manifest,n.url,c)}}async processOne(e,n,s,i){i?.(`Fetching ${e.url} via LLM\u2026`);let l=await rt(this.app,this.settings,e.url),r=this.settings.templates.find(x=>x.linkType===(l?.linkType??""))??this.settings.templates[0],c=n.get(r.linkType)??s,p=(l?.suggestedDestination||r.defaultDestination).trim(),u=e.title??e.url,b=at(V(l?.refinedTitle)||V(u)||""),y=ot(),m=`${y} - ${b||"Untitled Link"}.md`,h=`${p}/${m}`,f=lt(c,u,e.url,l,y,p),d=await this.resolveCollision(h,e.url);if(d.kind==="skip")return null;if(d.kind==="abort")return{abort:!0};let g=d.path;return await this.app.vault.create(g,f),g}async resolveCollision(e,n){if(!await this.app.vault.adapter.exists(e))return{kind:"write",path:e};let i=await new Promise(p=>{new F(this.app,{notePath:e,sourceUrl:n,onChoose:u=>p(u)}).open()});if(i==="skip")return new a.Notice(`Skipped duplicate: ${e}`),{kind:"skip"};if(i==="abort")return new a.Notice(`Aborted batch at duplicate: ${e}`),{kind:"abort"};if(i==="overwrite"){let p=this.app.vault.getAbstractFileByPath(e);return p instanceof a.TFile&&await this.app.vault.delete(p),{kind:"write",path:e}}let l=e.includes("/")?e.slice(0,e.lastIndexOf("/")):"",r=".md",c=e.slice(l.length+1,-r.length);for(let p=2;p<1e3;p++){let u=`${l}/${c} - ${p}${r}`;if(!await this.app.vault.adapter.exists(u))return new a.Notice(`Renamed to: ${u}`),{kind:"write",path:u}}return{kind:"write",path:e}}async listProjectTypes(){let e=this.app.vault.adapter,n=this.settings.projectsRoot.replace(/\/+$/,"");if(!await e.exists(n))return[];let s=await e.list(n);return(await Promise.all(s.map(async r=>{let c=r.isDirectory===!0||await e.exists(r.path);return{p:r,isDir:c}}))).filter(({isDir:r})=>r).map(({p:r})=>r).filter(r=>/^\d+\./.test(r.name)).sort((r,c)=>{let p=parseInt(r.name,10),u=parseInt(c.name,10);return p-u}).map(r=>({label:r.name,value:r.name,absPath:r.path}))}async createProjectFromSelection(e){let n=await this.listProjectTypes();if(n.length===0){new a.Notice(`No numbered subfolders under "${this.settings.projectsRoot}/". Add at least one (e.g. "1. Coding") and retry.`,1e4);return}let s=await new Promise(d=>{new N(this.app,{types:n,projectsRoot:this.settings.projectsRoot,templateScriptPath:this.settings.templateScriptPath,initialPlan:e,onDone:g=>d(g)}).open()});if(!s)return;let i=`${this.settings.projectsRoot}/${s.typeValue}/${s.name}`;new a.Notice(`Scaffolding ${i}\u2026`,5e3);let l=await ut();if(!l){new a.Notice("Could not find Python (tried: python, python3, py -3). Install Python or update PATH, then retry.",1e4),await $(this.app,this.manifest,"create-project-from-selection","Python not found on PATH");return}let r=this.app.vault.adapter.basePath.replace(/[/\\]+$/,""),c=r.includes("\\")?"\\":"/",u=[this.settings.templateScriptPath.startsWith(r)?this.settings.templateScriptPath:`${r}${c}${this.settings.templateScriptPath.replace(/\//g,c)}`,"--name",s.name,"--key",s.key,"--dst",i],b="",y="",m=-1;try{let d=await new Promise((g,x)=>{let D=require("child_process").spawn(l.bin,[...l.args,...u],{cwd:this.app.vault.adapter.basePath,env:{...process.env,OBSIDIAN_VAULT_PATH:this.app.vault.adapter.basePath},shell:l.shell}),I="",w="";D.stdout.on("data",v=>{I+=v.toString()}),D.stderr.on("data",v=>{w+=v.toString()}),D.on("error",v=>x(v)),D.on("close",v=>g({code:v??-1,stdout:I,stderr:w}))});m=d.code,b=d.stdout,y=d.stderr}catch(d){let g=d instanceof Error?d.message:String(d);new a.Notice(`Spawn failed: ${g}`,1e4),await $(this.app,this.manifest,"create-project-from-selection",`spawn failed: ${g}`);return}if(m!==0){let d=(y||b).slice(-800);new a.Notice(`init-project.py exited ${m}: ${d}`,12e3),await $(this.app,this.manifest,"create-project-from-selection",`exit ${m}: ${d}`);return}let h=`${i}/${s.name}.md`,f=this.app.vault.getAbstractFileByPath(h);f instanceof a.TFile?(await this.app.workspace.getLeaf(!1).openFile(f),new a.Notice(`Created ${i}`)):new a.Notice(`Scaffold succeeded but couldn't find ${h} \u2014 check the folder manually.`)}async moveToArchive(e){if(e.path.startsWith(this.settings.archiveRoot+"/")){new a.Notice(`Already in ${this.settings.archiveRoot}: ${e.path}`);return}let n=`${this.settings.archiveRoot}/${e.path}`;if(this.app.vault.getAbstractFileByPath(n)){new a.Notice(`Archive destination already exists: ${n}`);return}let i=await this.app.vault.read(e),l=this.injectArchivedAt(i);l!==i&&await this.app.vault.modify(e,l);let r=n.includes("/")?n.slice(0,n.lastIndexOf("/")):"";r&&!await this.app.vault.adapter.exists(r)&&await this.app.vault.adapter.mkdir(r);try{await this.app.fileManager.renameFile(e,n),new a.Notice(`Archived \u2192 ${n}`)}catch(c){let p=c instanceof Error?c.message:String(c);new a.Notice(`Archive failed: ${p}`,8e3)}}injectArchivedAt(e){let s=`archivedAt: ${new Date().toISOString().replace(/\.\d+Z$/,"Z")}`,i=e.match(/^---\r?\n([\s\S]*?)\r?\n---/);if(!i)return`---
${s}
---
